...
public class DbIndexer : IDbIndexer {
    ...
    public async Task<int> ReIndexAsync(DbConnectionString dbConnectionString) {
        using (await _asyncLock.LockAsync()) {
            var dbid = await _schemaSearchRepository.DbExistsAsync(dbConnectionString);
            if (dbid > 0) {
    ...
    public async Task<int> DbExistsAsync(DbConnectionString dbConnectionString) => await _schemaSearchRepository.DbExistsAsync(dbConnectionString);
    ...
    public async Task<int> IndexAsync(DbConnectionString dbConnectionString) {
        ...
        using (await _asyncLock.LockAsync()) {
            ...
            var dbObjectsQuery = """
            WITH DefinitionInfo (object_id, definition) AS (  
            ...
            JOIN sys.objects t           ON ind.object_id = t.object_id 
            WHERE  
                t.is_ms_shipped = 0 
            ORDER BY 
            ...
            return await _schemaSearchRepository.InsertDbAsync(dbDefinition, dbobjects, dbColumns, dbIndices, dbIndicesColumns);
        }
    }
}
